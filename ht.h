...
public:
    ...
    HASH_INDEX_T next() 
    {
        if (this->numProbes_ == this->m_){
            ...
            HASH_INDEX_T holder = this->numProbes_*this->dhstep_;
            holder += this->start_;
            holder %= this->m_;

            this->numProbes_++;
            return holder; // Might need to move return outside of else
        }
    }
};
...
{
    HashItem * val = internalFind(p.first);
    double loadF = static_cast<double>(countItems) / CAPACITIES[mIndex_];
  
    if (val != nullptr || newAlpha_ <= loadF) {
    ...
    HASH_INDEX_T itemIndex = this->probe(p.first);
    if (itemIndex == npos) {
        throw std::logic_error("No empty slot");
    } else {
        HashItem* newItem = nullptr;
        if (table_[itemIndex] == nullptr) {
            //std::cout << "adding item" << std::endl;
            newItem = new HashItem(p);
            table_[itemIndex] = newItem;
        } else {
            table_[itemIndex]->item.second = p.second;
        }
    }
    countItems++;
    count++;
...
{
    HashItem * holder = internalFind(key);
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
    ...
    ++mIndex_;
    std::vector<HashItem*> tableHolder = table_;
    ...
    for (auto& item : tableHolder) {
        if (item != nullptr) {
            std::cout << "Key" << item->item.first << ", Val " 
            << item->item.second << std::endl;
        }
    }
    ...
    */
    ...
    //std::cout << "work plssss" << std::endl;
    for (auto it = tableHolder.begin(); it != tableHolder.end(); ++it) {
        // std::cout << "got into the loop" << std::endl;
        if (*it != nullptr && !(*it)->deleted) {
            //std::cout << "just inserted" << std::endl;
            insert((*it)->item);
        }
        delete *it;
    }
